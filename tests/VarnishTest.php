...
it('can generate a ban command for a single host', function () {
...
it('can generate a ban command for a single host and a specific url', function () {
    $expectedCommand = "sudo varnishadm -S /etc/varnish/secret -T 127.0.0.1:6082 'ban req.http.host ~ (^example.com$) && req.url ~ /nl/*'";

    expect((new Varnish())->generateBanCommand(['example.com'], '/nl/*'))->toEqual($expectedCommand);
});
